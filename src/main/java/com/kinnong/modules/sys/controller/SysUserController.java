...
public class SysUserController {
    ...
    @AuthIgnore
    public R getInfo(@RequestAttribute Long userId, HttpServletRequest request) {
        userId = (Long) request.getAttribute("userId");
        return R.ok().put("user", sysUserService.queryObject(userId));
    }
    @RequestMapping("/getInfo1")
    @AuthIgnore
    public R getInfo1(@RequestParam("userId") String userId, HttpServletRequest request) {
        return R.ok().put("user", sysUserService.queryObject(Long.valueOf(userId)));
    }
